...
- 추천 파이프라인은 동작 중이지만, 최근 운영 구간에서는 Spotify `/search` 429(요청 한도 초과)로 인해 `partial` 결과 비중이 높습니다.
- 현재는 429 연속 발생 시 **검색 제한 모드**로 자동 전환되어 요청량을 줄이고 fallback으로 사용자 흐름을 유지합니다.
- 프롬프트 분석(Gemini) 자체는 정상 동작하며, 병목은 주로 Spotify 검색 수집 단계에서 발생합니다.
...
6. 429 대응 검색 감속/쿨다운/회로차단(서킷) 로직
7. fallback 결과 중복곡 억제(반복 삽입 방지 + 최근 히스토리 회피)
...
- 429 연속 발생 시 검색 제한 모드 전환
- 제한 모드에서 쿼리 수/동시성/쿼리별 limit 자동 감속
- 반복 429 시 optional query 스킵 및 조기 중단으로 타임아웃 연쇄 완화
- `partialPool`이 비어도 즉시 중단하지 않고 bootstrap fallback으로 결과 반환
...
- 🔄 Spotify 검색 한도 구간에서 프롬프트 반영률을 유지하기 위한 수집/복구 전략 고도화 진행 중
...
- `EXPO_PUBLIC_GEMINI_PROXY_URL_DEV` (개발용 native/web proxy URL)
- `EXPO_PUBLIC_GEMINI_PROXY_URL_PROD` (배포용 native/web proxy URL)
- `EXPO_PUBLIC_GEMINI_PROXY_URL` (선택: 레거시 fallback)
...
## 13. 📈 운영 로그 해석 가이드 (현재 버전)

- `Gemini analyzePrompt success`:
  - 프롬프트 해석은 정상 완료.
- `Spotify request failed (429)` / `search rate-limited give-up`:
  - Spotify 검색 한도 초과 구간.
- `search limited mode`:
  - 자동 감속 모드 진입(쿼리 수/동시성 축소).
- `optional skipped after guaranteed`:
  - 한도 보호를 위해 보조 검색 생략.
- `partialPool size=0`:
  - 실시간 검색 수집 결과가 비어 있음.
- `fast_timeout_bootstrap_fallback`:
  - 실시간 수집 실패 시 bootstrap 기반 fallback 반환.
- `status=partial`:
  - 실패가 아닌 축약 결과 반환 상태.

현재 알려진 제약:
- Spotify 검색 한도 구간이 길어지면, 프롬프트 기반 신규 후보 수집이 약해져 기존 fallback 풀 곡이 반복 노출될 수 있습니다.
- 앱 모드/권한 상태에 따라 Spotify recommendations endpoint가 제한될 수 있어, search 의존도가 높아질 수 있습니다.
